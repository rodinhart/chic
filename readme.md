...
The language Chic was written in the spirit of [Nile](https://github.com/damelang/nile) in order to understand Nile better. The tokenizer, parser and interpreter live in chic.js, while the main demonstration resides in main.chic.
...
will expose index.html/index.js which will evaluate main.chic. The result will be that several operators (functions) will be added to the interpreter environment. The complete table of operators is logged to the console, and two demo functions are will be used to display a spinning cube, and, if enabled in index.js, the Mandelbrot set. By default the Mandelbrot set is disabled, as it is quite slow.
...
**`project scale ∈ Number ≡ ∀ V >> scale × (Point V.x / V.y  V.z / V.y)`** Implements the operator project which takes a scaling factor and returns a pipe that projects a stream of vectors onto points. Read right hand side as "For all V, stream out the result of expression scale x ..." etc.
